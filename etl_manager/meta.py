...
class TableMeta:
    ...
    def _check_valid_data_format(self, data_format):
        if data_format not in _supported_data_formats:
            ...
            raise ValueError(
                f"The data_format provided ({data_format}) must match the supported "
                f"data_type names: {sdf}"
            )
    ...
    def _check_valid_datatype(self, data_type):
        if not data_type_is_regex(data_type):
            ...
            raise ValueError(
                f"The data_type provided must match the supported data_type names: "
                f"{scf}"
            )
    ...
    def _check_column_exists(self, column_name):
        if column_name not in self.column_names:
            ...
            raise ValueError(
                f"The column name: {column_name} does not match those existing in "
                f"meta: {cn}"
            )
    ...
    def update_column(self, column_name, **kwargs):
        ...
        if len([k for k in kwargs.keys() if k in _column_properties]) == 0:
            raise ValueError(
                f"one or more of the function inputs ({', '.join(_column_properties)}) "
                f"must be specified."
            )
    ...
    def glue_table_definition(self, full_database_path=None):
        ...
        if full_database_path:
            ...
            raise ValueError(
                "Need to provide a database or full database path to generate glue "
                "table def"
            )
    ...
    def generate_markdown_doc(self, filepath):
        ...
        f.write(f"\n")
        f.write(
            "*Note: This meta data document has been automatically generated by the "
            "etl_manager package*"
        )
        f.write("\n")
        f.write("## Details")
        ...
        if self.partitions:
            if not temp_athena_staging_dir:
                if self.database:
                    ...
                    raise ValueError(
                        "You must provide a path to a directory in s3 for Athena to "
                        "cache query results"
                    )
            ...
            if not database_name:
                if self.database:
                    ...
                    raise KeyError(
                        "You must provide a database name, or register a database "
                        "object against the table"
                    )
